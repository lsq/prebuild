...
const cwd = path.join(__dirname, 'native-module-napi-gyp')
const osType = require('os').type()

test('can prebuild a gyp napi module for node', function (t) {
  fs.rmSync(path.join(cwd, 'prebuilds'), { recursive: true, force: true })
  const file = 'native-v1.0.0-napi-v1-' + process.platform + (osType.startsWith('MINGW32_NT') ? 'gnu' : '') + '-' + process.arch + '.tar.gz'
